...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, out_layers: list[int] | tuple[int, ...] | None = None):
        if out_layers is None:
            return self.resblocks(x)

        out_tokens = []
        selected_layers = set(out_layers)
...
class VisionTransformer(nn.Module):
    ...
    def forward(
        self,
        x: torch.Tensor,
        features_list: list[int] | tuple[int, ...] | None = None,
    ):
        ...
        if features_list is None:
            ...
            x, patch_tokens = self.transformer(x, features_list)
...
class CLIP(nn.Module):
    ...
    def encode_image(
        ...
        return self.visual(image.type(self.dtype), features_list=features_list)
