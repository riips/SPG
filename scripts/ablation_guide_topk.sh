...
for i in "${!train_data_list[@]}"; do
    ...
    for guide_topk in 8 16 32 64 128 256 512 1024; do
        echo "guide_topk=${guide_topk}"
        ...
        python run.py mode=train model=facebook_dinov3-vitl16 experiment=guide_sae data=single_dataset/${train_data} train.epoch=${guide_epoch} \
            ...
            model.method_config.guide_sae.sparsity_type="topk" \
            model.method_config.guide_sae.guide_topk=${guide_topk}
