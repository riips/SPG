...
for e in "${!encoders[@]}"; do
    encoder=${encoders[e]}
    image_size=${image_sizes[e]}
    echo "encoder=${encoder} image_size=${image_size}"
    ...
    for i in "${!train_data_list[@]}"; do
        ...
        # train sae
        python run.py mode=train_sae model=${encoder} data=single_dataset/${train_data} train.epoch=${sae_epoch} \
        ...
        python run.py mode=train model=${encoder} experiment=guide_sae data=single_dataset/${train_data} train.epoch=${guide_epoch} \
            model.image_size=${image_size} \
            ...
            model.method_config.guide_sae.model_name=${encoder} \
